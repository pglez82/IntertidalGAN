...
from tidegan_dataset import TideGANDataset, SITES, SITE_TIDE_RANGES
...
def get_config(args):
    ...
    config = {
        ...
        "site": args.site,
        "n_sites": len(args.site) if args.site else len(SITES),
        "cond_dim": 16,
        "beta1": 0.5,
        "beta2": 0.999,
...
def train(args):
    ...
    if args.resume:
        start_epoch, global_step = load_checkpoint(
            args.resume, G, D, optimizer_g, optimizer_d, device)
...
def generate_images(args):
    ...
    site = args.site  # must be capitalized e.g. "Foz"
    site_lower = site.lower()
    csv_path = f"/home/pablo/Proyectos/IntertidelGEN/data/{site}/dataset_{site_lower}.csv"
    import csv as csv_mod
    with open(csv_path) as f:
        rows = list(csv_mod.DictReader(f))
    rows_sorted = sorted(rows, key=lambda r: float(r["marea_m"]))

    ref_path = f"/home/pablo/Proyectos/IntertidelGEN/data/{site}/{rows_sorted[0]['imagen_rgb']}"
